...
| <a name="input_domain_names"></a> [domain\_names](#input\_domain\_names) | Fully-qualified domain name(s) to register. The domain names to use for API Gateway - will use the index of stage\_names to select the domain name.<br/><br/>Usage modes:<br/>- Set to null or [] to skip custom domain configuration (use default AWS endpoint)<br/>- Provide list of domain names when create\_api\_domain\_name = true to create new custom domains<br/>- Provide list of domain names when create\_api\_domain\_name = false to use existing custom domains | `list(string)` | `null` | no |
