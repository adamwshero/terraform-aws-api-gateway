...
| <a name="input_domain_names"></a> [domain\_names](#input\_domain\_names) | The domain names to use for API gateway it will use the index of stage\_names to select the domain name | `list(string)` | `null` | no |
| <a name="input_domain_ownership_verification_certificate_arn"></a> [domain\_ownership\_verification\_certificate\_arn](#input\_domain\_ownership\_verification\_certificate\_arn) | ARN of the AWS-issued certificate used to validate custom domain ownership (when certificate\_arn is issued via an ACM Private CA or mutual\_tls\_authentication is configured with an ACM-imported certificate.) | `string` | `null` | no |
